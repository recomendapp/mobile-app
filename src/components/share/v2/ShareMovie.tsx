...
>(({ movie, variant = 'default', isPremium, ...props }, ref) => {
	...
	const renderEditButtons = useMemo(() => (
		<View style={tw`absolute top-2 right-2 flex-row items-center gap-2`}>
			{editing && backdrop && activeEditingOption === 'background' && isPremium && (
				<Animated.View entering={FadeInRight} exiting={FadeOutRight}>
					<Button
					variant="muted"
					icon={bgType === 'color' && bgColor ? CircleIcon : Icons.Image}
					size="icon"
					iconProps={bgType === 'color' && bgColor ? { fill: bgColor.color } : undefined}
